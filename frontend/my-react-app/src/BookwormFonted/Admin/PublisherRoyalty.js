...
const PublisherRoyalties = () => {
  const [Publishersroyalti, setPublisherroyaties] = useState([]);
  const { id } = useParams();
 const{name} = useParams();
  ...
  return (
    <div>
      <h3> Royaltie of Publisher: {name}</h3>
      <Table striped bordered hover variant="wh">
        ...
        <tbody>
          {Publishersroyalti.map((item, index) => (
            <tr key={index}>
              <td>{index + 1}</td>
...
export default PublisherRoyalties;
